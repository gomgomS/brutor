...
class konten_proc:
    ...
    def _add(self, params):
        response = helper.response_msg(
            "ADD_KONTEN_SUCCESS", "ADD KONTEN SUCCESS", {} , "0000"
        )
        try:
            ...
            konten_rec  = database.new(self.mgdDB, "db_konten")
            konten_rec.put("konten_id",                 konten_rec.get()["pkey"             ])
            konten_rec.put("fk_user_id",                params["fk_user_id"                 ])
            konten_rec.put("fk_perusahaan_id",          params["fk_perusahaan_id"           ])
            konten_rec.put("fk_posisi_value",           params["fk_posisi_value"            ])
            konten_rec.put("fk_jenis_tampilan_value",   params["fk_jenis_tampilan_value"    ])
            konten_rec.put("perusahaan_name",           partner_rec["nama_perusahaan"       ])
            konten_rec.put("posisi_name",			    posisi_rec["name"                   ])
            konten_rec.put("jenis_tampilan_name",       jenis_tampilan_rec["name"           ])
            konten_rec.put("urutan",                    int(params["urutan"                 ]))
            konten_rec.put("judul_konten",              params["judul_konten"               ])
            konten_rec.put("deskripsi",			        params["deskripsi"                  ])
            konten_rec.put("deskripsi_html",		    deskripsi_html                       )
            konten_rec.put("deskripsi_preview",         deskripsi_preview                   )
            konten_rec.put("url",                       params["url"                        ])
            konten_rec.put("start_timestamp",           start_timestamp                      )
            konten_rec.put("end_timestamp",             end_timestamp                        )
            konten_rec.put("str_start_datetime",        params["start_datetime"             ])
            konten_rec.put("str_end_datetime",          params["end_datetime"               ])
            konten_rec.put("image",                     file_image                          )
            konten_rec.put("status_value",              "PROSES")
            konten_rec.insert()

            
            # SAVE IMAGE RECORD
            konten_pkey = konten_rec.get()["pkey"]

            image_rec  = database.new(self.mgdDB, "db_konten_img")
            image_rec.put("fk_konten_id",      konten_pkey  )
            ...
            status_history_rec.put("fk_konten_id",  konten_pkey           )
            ...
            response.put( "status"      , "ADD_KONTEN_FAILED" )
            response.put( "desc"        , "ADD KONTEN FAILED" )
