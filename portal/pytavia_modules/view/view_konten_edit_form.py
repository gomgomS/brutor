...
class view_konten_edit_form:
    ...
    def _find_one_konten(self, params):
        konten_rec = self.mgdDB.db_konten.find_one({"pkey" : params["fk_konten_id"], "is_deleted" : False })

        image_rec = self.mgdDB.db_konten_img.find_one({"fk_konten_id" : params["fk_konten_id"], "is_deleted" : False }) 
        if image_rec != None:
            image_rec["image"] = config.G_IMAGE_URL_DISPATCH + image_rec["image"]
        else:
            image_rec = {
                "pkey"  : "",
                "image" : "../../static/assets/img/placehold.jpg"
            }

        response = {
            "konten_rec"    : konten_rec,
            "image_rec"     : image_rec
        }
        return response
    # end def
    ...
    def html(self, params):
        response = helper.response_msg(
            "FIND_KONTEN_EDIT_FORM_SUCCESS", "FIND KONTEN EDIT FORM SUCCESS", {} , "0000"
        )
        try:
            ...
            # FIND ONE KONTEN
            konten_resp             = self._find_one_konten( params )
            konten_rec              = konten_resp["konten_rec"  ] 
            image_rec               = konten_resp["image_rec"   ] 
            ...
            html = render_template(
                "pengelolaan_konten/konten_edit_form.html",
            ...
            response.put( "status"      , "FIND_KONTEN_EDIT_FORM_FAILED" )
            response.put( "desc"        , "FIND KONTEN EDIT FORM FAILED" )
