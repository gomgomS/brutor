...
class view_topup_history:
    ...
    def _find_topup_history(self, params):
        #PAGINATION
        page            = params["page"         ]
        ...
        topup_history_list = []
        topup_history_view = self.mgdDB.db_topup_request.find(query).sort('rec_timestamp', order)
        block_count = utils.ceildiv(topup_history_view.count(), entry)
        ...
        for topup_history_item in topup_history_view:
            find_name_user                      = self._get_requester_information(topup_history_item["request_user_id"] )            
            topup_history_item["name"]          = find_name_user["name"     ]

            # Convert amount to Rupiah currency format
            if 'amount' in topup_history_item: 
                topup_history_item['amount'] = self.format_currency(topup_history_item['amount'])

            topup_history_list.append(topup_history_item)

        response = {
            "topup_history_list"   : topup_history_list,
            "block_count"   : block_count,
    ...
    def html(self, params):
        response = helper.response_msg(
            "FIND_topup_history_PROSES_SUCCESS", "FIND KONTEN PROSES SUCCESS", {} , "0000"
        )
        try:
            ...
            # FIND topup_history
            topup_history_resp             = self._find_topup_history( params )
            topup_history_list             = topup_history_resp["topup_history_list"     ]
            block_count             = topup_history_resp["block_count"     ]
            prev_button             = topup_history_resp["prev_button"     ]
            next_button             = topup_history_resp["next_button"     ]

            
            html = render_template(
                "payment/topup_history_list.html",
    ...
    def _get_requester_information(self,user_id):              
