...
class view_register_meeting:
    ...
    def _find_register_meeting(self, params):
        #PAGINATION
        page            = params["page"         ]
        ...
        # konten_list = []
        # konten_view = self.mgdDB.db_meeting.find(query).sort(order).skip(block_skip).limit(entry)
        ...
        for meeting_item in meeting_view:            
            class_resp                          = self._find_activation_class( meeting_item["activation_class_id"] )
            meeting_item["active_class_name"]   = class_resp["active_class_name"     ]
    ...
    def html(self, params):
        ...
        try:
            ...
            print(meeting_resp)
            meeting_list              = meeting_resp["meeting_list"     ]
            block_count             = meeting_resp["block_count"     ]
            prev_button             = meeting_resp["prev_button"     ]
            next_button             = meeting_resp["next_button"     ]

            
            html = render_template(
                "register_meeting/register_meeting_list.html",
