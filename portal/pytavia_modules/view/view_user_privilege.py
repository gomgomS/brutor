...
class view_user_privilege:
    ...
    def show_menu_list(self, params):
        ...
        if "id" in params: 
            value = params["id"]
        # end if
        edit_item_rec = database.get_record("db_config_privilege")
        if value != None:
            edit_item_rec = self.mgdDB.db_config_privilege.find_one({
                "value" : value
            })
        # end if
        config_all_rec        = self.mgdDB.db_config_all.find_one({"value" : page_id})
        config_privilege_view = self.mgdDB.db_config_privilege.find({})
        config_privilege_list = list( config_privilege_view )
        response = {
            "config_privilege_list" : config_privilege_list,
    ...
    def html(self, params):
        ...
        config_privilege_list  = show_menu_data["config_privilege_list"] 
        edit_item_rec          = show_menu_data["edit_item_rec"        ] 
        config_all_rec         = show_menu_data["config_all_rec"       ] 
        return render_template(

            "config-privilege/privilege-list.html",
