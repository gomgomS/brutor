...
class view_tutor_approval:
    ...
    def _find_tutor_approval(self, params):
        #PAGINATION
        page            = params["page"         ]
        ...
        query = { 
            # "is_deleted"        : False, 
            # "status_value"      : "PROSES"
            "summery_status_applying": {"$in": ["WAITING", "SET MEETING", "APPROVE"]},
            # "register_teacher"  : 'FALSE'
        }
        ...
        # konten_list = []
        # konten_view = self.mgdDB.db_user.find(query).sort(order).skip(block_skip).limit(entry)
    ...
    def html(self, params):
        response = helper.response_msg(
            "FIND_user_SUCCESS", "FIND user SUCCESS", {} , "0000"
        )
        try:
            ...
            next_button             = user_resp["next_button"     ]

            
            html = render_template(
                "tutor_approval/tutor_approval_list.html",
